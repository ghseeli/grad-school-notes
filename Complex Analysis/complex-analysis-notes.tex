...
\section{(11/10/2016)}
  ...
  rods. This is being ommitted. 

  \begin{thm}[Open Mapping Theorem for analytic functions]
    The image of an open set under an analytic function is open. (Did
    this in homework.)
  \end{thm}
  \begin{proof}
  ...
  \end{prop}
  This proposition follows from the fact htat $f(z)-w_0 =
  ...
  Let $\gamma = \gamma(t), a < t < b$ be a parametrized closed curve
  ...
  \begin{proof}
    ...
    \end{align*}
  \end{proof}
  If $D$ is undbounded, one can shrink to a bounded domain containing
  $\gamma$ (the argument is in the textbook.)
  \begin{cor}[Cauchy Integral Formula +]
  ...
  \begin{thm}
    All conformal equivalences from the unit circle ot the unit circle
    have the form \[
  ...
  \begin{proof}
    ...
    be confromal sending $a \mapsto 0$. Consider $f \circ h^{-1}$ a
    confromal equivalence sending $0 \mapsto 0$. The Schwarz
  ...
  \begin{proof}
    ...
    \frac{1-f(z_1)}{1-\overline{f(z_1)}z}$. Then, we apply the Schwarz
 ...
 these values. 
  ...
  \]
  This gives us an infinitesmal version, as well, which is much easier
  to remember, namely \[
  ...
  \begin{itemize}
  \item The geodesics are circles (or lines through infinity) that
  ...
  \begin{defn}
    ...
    \]
    and thus the infinitesmal version is given by
    $\frac{2|dz|}{1-|z|^2}$.
  \end{defn}
  ...
  of a path $\gamma$ is \[
    ...
    \frac{|\gamma'(t)|}{1-|\gamma(t)|^2}dt = \log \left(
      \frac{1+\rho_H(z,w)}{w-\rho_H(z,w)} \right) = 2 \tanh^{-1}(\rho_H(z,w))
  \]
  ...
  confromal self-maps of $\D$ are in bijection with the isometries of
  ...
  \begin{defn}
    ...
    compiosition) of finitely many conformal automorphisms $\D \to
    \D$, ie \[
  ...
  \begin{proof}
    ...
    limite of this sequence, $f$, satisfies the following
    \begin{itemize}
    \item $f(z_0) = 0$
    \item $|f'(z_0)| = \sup > 0$
