...
\end{rmk*}
    ...
    \begin{tabular}{|c|c|}
        \hline
        $\frac{1}{z}$ & Singly valued on $\C \textbackslash \{0\}$, 0 is not a
        branch point. \\
    ...
    \begin{proof}
        ...
        (u(x,y)+iv(x,y))}{\Delta x} = u_x(x,y)_iv_x(x,y)$. 
    ...
    yet. Note also that if $f = u_iv$ is analytic, then $u$ is harmonic. This
    follow because $u_{xx}+u_{yy} = u_{xx}+(-v_x)_y = u_{xx}-v_{yx} =
    ...
    is, on a neighborhood of any point, there is a $v$ with $u+iv$ analytic.
    Such a $v$ is called a \emph{harmonic conjugate} (unique up to additive
    constant.) eg, there is an $x$ such that $u=xy$ satisfying \[ 
        \begin{cases}
 v_x = -u_y = x \\
 v_y = u_x = y.
    \end{cases}
