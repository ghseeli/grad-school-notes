...
\item[(b)] If $I_1,\dots,I_n$ are pairwise comaximal, then of course $\ker \phi = \prod_{J=1}^n I_j$, $\phi$ is surjective, and thus
	\[R / \prod_{j=1}^n \cong \bigtimes_{j=1}^n R/I_j \]
\end{enumerate}
\end{proposition}
...
\begin{remark*}
A special case of the CRT os when $R = \z$.
\begin{enumerate}
...
Assume $0 \ne f \in R[x]$, with $\ell(f) \in R\x$. Then for any $g \in R[x]$, there exist uniquely determined $q,r \in R[x]$ such that $g = fq +r$, where $\deg r < \deg f$ (possibly $r=0$).
\end{proposition}
...
\begin{corollary}
If $F$ is a field and $0 \ne f \in F[x]$, then for all $g \in F[x]$, there exist uniquely determined $h,r \in F[x]$ such that $g = fh + r$ with $\deg r < \deg f$.
\end{corollary}
...
\begin{proof}
We combine 2.1.13 with 1.2.9 (b). Let $A \leq F\x$ be a subgroup of $F\x$ with order $n \in \N$. Note, of course, that $A$ is abelian. Fix $m \in \N$. Then for all $a \in R$, $a^m1 \iff a^m-1=0 \iff a$ is a root of $f := x^m-1\in R[x]$. By 2.1.13,
...
\begin{enumerate}
\item[(a)] $R = \z_9 \implies R\x = \{1,3,5,7\} \cong \z_2 \times \z_2$.
...
\begin{defn}
A \tb{Euclidean domain} is an integral domain $R$ which admits a (degree, norm) function $d : R\bs\{0\} \ra \N_0$ such that for all $a,b \in R$, $a \ne 0$, there exist $q,r \in R$ such that $b = qa + r$ with $r = 0$ or $d(r) < d(a)$. \\
...
\begin{enumerate}
\item[(a)] $0 = \gcd(0,0)$.
\item[(c)] If $c = \gcd(a,b)$, then $uc = \gcd(a,b)$ for any $u \in R\x$.
...
\vdots & &&\vdots \\
r_{i-1}q_{i-1} &= q_i r_i + r_{i+1},   && d(r_{i+1})<d(r_i), \\
\vdots & && \vdots \\
...
\item[(c)] Assume that for all $z \in K$ there exists $q \in R$ such that $N(z-q) < 1$, then $R$ is Euclidean with $d(a) := |N(a)|$ for $a \in R$.
\end{enumerate}
\end{lemma}
\begin{proof}
\
...
Then it is easy to show (exercise) that if for $a,b \in R\bs\{0\}$ with $a = u \prod_{i \in I} p_i^{e_i}, b = \prod_{i \in I} p_i^{f_i}$, then
\begin{enumerate}
...
\begin{remark}
Le $F$ be a field and $F \in F[x]$. $f$ is irreducible if and only if $\deg f \geq 1$ and for all $g \in F[x]$, $g \mid f$ implies that $\deg g = 0$ or $\deg g = \deg f$. In particular, if $\deg f = 1$, then $f$ is irreducible, and if $\deg f = 2,3$, then $f$ is irreducible if and only if $f$ has no roots in $F$.
\end{remark}
