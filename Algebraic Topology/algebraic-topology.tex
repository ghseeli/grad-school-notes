...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $$\rhoml(A \cap B) \isomto \rhoml(A) \dsum \rhoml(B)$$
    or
    $$\rhoml(X) \isom \rhoml(A) \dsum \rhoml(B).$$

    Thus if $c \in \rchains(X)$ represents a nontrivial homology class (hollow cycle), then it must be in $\rchains(A)$ or $\rchains(B)$.

    WLOG, it's in $\rchains(A)$.  Now
    $$ [0, \tfrac 12] \x D^{k-1} = [0, \tfrac 14] \x D^{k-1} \union [\tfrac 14, \tfrac 12] \x D^{k-1}.$$

    Applied inductively, there exists a sequence of intervals $(I_l)_{l=1}^{\oo}$ of length $(\tfrac 12)^l$ s.t. $I \supset I_1 \supset I_2 \supset I_3 \supset \supset \supset$ and
    \begin{align*}
    ...
    where all the $\rhoml$'s above are nonzero except for the last.

    There exists a $b$ in $\rchains[m+1](X_\oo)$ s.t. $c = \d b$.  $b$ is a finite sum of $\Delta^{m+1} \to X_\oo = S^n - h(\{q\} \x D^{k-1})$

    For any metric on $S^n$, there exists an $\epsilon$ s.t. $d(supp(b), h(\{q\} \x D^{k-1})) > \epsilon > 0$.

    Thus $b \in \rchains[m+1](X_l)$!

    $X_\oo \minus X_l = h(I_l \minus \{q\} \x D^{k-1})$

    as $l \to \oo$

    $\max_{y \in I_l \x D^{k-1}} dist(y, \{d\} \x D^{k-1}) \to 0$

    That is, for $n$ sufficiently large, $X_l \union \{\text{-neighborhood around $S^n \minus X_\oo$}\} = S^n$.
