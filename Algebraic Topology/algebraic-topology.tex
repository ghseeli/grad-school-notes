...
\begin{rmk}
  ...
  \begin{itemize}
    \item A two cycle
    \item A 1 cycle
    \item A path of length 1
  \end{itemize}
  But they still qualify as simplicial complexes.
\end{rmk}

\begin{defn}
  Note: I don't think this is defined rigorously, and should be double-checked.
  An \de{$n$-chain} is a union (with multiplicity, and negative multiplicity) of $n$-simplices.
\end{defn}
\begin{rmk}
  An $n$-chain is a subspace of dimension $n$.

  Also, negative multiplicities represent the opposite orientation of their positive counterparts.
\end{rmk}

An orientation of an $n$-simplex is specified by an order on the vertices.

Also, an even permutation (on these vertices) is orientation preserving, while an odd permutation is orientation reversing.

Convention: faces are orientated accoring to the induced order from their vertices.
...
Remember, i want the boundary of an $n$-chain.

\begin{defn}
  The boundary map $\d \from \delta_n(X) \to \delta_{n-1}(X)$ is the linear map $$\d(\sigma) = \sum_{i=0}^n (-1)^i \sigma|_{[0, 1,,,, \hat{i},,, n]}$$
\end{defn}

Houston we have a problem.  The induced orientation on the boundary doesn't neccessarily agree with the original orientation on the those lower dimensional simplices! (see above?)
...
\begin{example}
  ...
  where $[0, 1, 2]$, for example, represents the triangle (path closed into a cycle) 0-->1-->2.
\end{example}
