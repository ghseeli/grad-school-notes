...
If we have \(X=\text{int} A \cup \text{int} B\), then the inclusion \((B, A \cap B) \righthookarrow (X,A)\) induces an isomorphism on homology groups as well.
...
S is the "Slicer".  It cuts up a linear simplex.  It barycenter subdivides it!  It is defined recursively.
...
\begin{example}
  ...
  S(I) = b(S \d I) = b(S ([v0] - [v1])) = b([v1] - [v0]) = [b,v1] - [b,v0].
\end{example}
...
\end{thm}
\begin{proof}
  ($\lambda$ is the input variable)

  $\d S = \d b (S \d \lambda) = S \d \lambda - b \d S \d \lambda$

  apply induction on $n$

  $= S \d \lambda - b S \d^2 \lambda = S \d \lambda$.
\end{proof}
\begin{defn}
  T \from LC_n(V) \to LC_{n+1}(V)
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $\d T \lambda = \d b(\lambda - T \d \lambda)
...
$S \sigma = \sum_{} \sigma |_{\text{simplices in subdivision}} = \sigma_{\#} S \delta^n$

again,
$S \d = \d S$ and $\d T + T \d = 1 - S$
