...
\newcommand{\homlindexdefault}{n}
\newcommand{\fund}[1][1]{\pi_{#1}}
\newcommand{\homl}[1][\homlindexdefault]{H_{#1}}
\newcommand{\rhoml}[1][\homlindexdefault]{\widetilde{H}_{#1}}
\newcommand{\dhoml}[1][\homlindexdefault]{\homl[#1]^\Delta}
\newcommand{\cwhoml}[1][\homlindexdefault]{\homl[#1]^{CW}}
\newcommand{\chains}[1][\homlindexdefault]{C_{#1}}
\newcommand{\rchains}[1][\homlindexdefault]{\widetilde{C}_{#1}}
\newcommand{\D}{$\Delta$-}
...
\title[Algebraic Topology]{Algebraic Topology MATH 7800 \\ Notes from
  a class by Dr. Ben Webster}
...
\begin{defn}
  A \de{homotopy} from \(f \in \Map(M, N)\) to \(g \in \Map(M, N)\) is a path in $\Map(M, N)$ that starts at $f$ and ends at $g$.  Therefore, $h_0 = f$ and $h_1 = g$.  (Note that we have not yet defined ``path'', and so you may use the following equivalent definition instead.)

  Equivalently, a \de{homotopy} from \(f \from M \to N\) to \(g \from M \to N\)
  is a map \(h\from I \x M \to N\) such that \(h_0=f\) and \(h_1=g\). We say \(f\) and
  \(g\) are \de{homotopic} if a homotopy between them exists.
...
When there are no restrictions placed on a homotopy, it is sometimes called a \de{free} homotopy.

\begin{defn}
  A \de{path} in a topological space $X$ is a map
  \[f \from I \to X.\]
\end{defn}
...
\begin{defn}
  ...
  This is called the \de{concatenation} or \de{path composition}
  of $f$ and $g$.
...
\begin{defn}[\cite{ncatlab}]
  ...
  \begin{itemize}
    ...
    \item For every pair of objects $A, B \in \Ob(\mathcal{C})$, a set $\Hom(A, B)$ of \de{morphisms}
...
\begin{defn}
  A \de{covering space} \((\widetilde{X},p)\) of a space \(X\) is a space \(\widetilde{X}\)
...
\begin{defn}[local property]
  ...
  Equivalently, $S$ is \de{locally} $P$ if every open cover of $S$ has a refinement whose open subsets are $P$.
\end{defn}
...
\end{thm}
\begin{defn}
  Given spaces $X$ and $Y$, we call the map $f\from X \to Y$ a \de{homotopy equivalence} if there exists $g\from Y \to X$ s.t. $[f.g] = [\id_X]$ and $[g.f] = [\id_Y]$.  We say that the spaces $X$ and $Y$ are \de{homotopically equivalent}. (See~\cite{heq} for more detail.)
\end{defn}
\begin{rmk}
...
\begin{defn}
  Given a space $X$, then paths $p$ and $q$ are \de{close} if $p^{-1}*q$ contracts into a neighborhood of $x'$ and $x''$.
\end{defn}
...
\begin{example}[Something that does not admit a \(CW\) structure]
  The topological space known as the \de{Hawaiian earring} is
...
\begin{defn}
  The \de{connected sum} of two topological spaces \(X\) and \(Y\)
...
\begin{defn}
  The $n$th \de{simplicial homology group} of $X$ is defined by
...
\begin{example}
  ...
  \begin{tikzcd}
    &\Delta_2(K) \arrow[r, "\d_2"] &\Delta_1(K) \arrow[r, "\d_1"] &\Delta_0(K) \\
    &\Z^2 \arrow[r, "M"] &\Z^3 \arrow[r, "0"] &\Z
  \end{tikzcd}
  where $M = \begin{bmatrix}
    1 &1 \\
    -1 &-1 \\
    1 &-1
  \end{bmatrix}$ and $0 = \begin{bmatrix}
    0 & 0 & 0
  \end{bmatrix}$.

  Observe that $\d(U) = -a -b +c$ and $\d(L) = a - b - c$, so that $\d(U + L) = -2b$.  On the other hand, we know the Klein bottle is closed, so that $\d(U + L) = 0$.  This gives us $-2b \sim 0$, but $b \not\sim 0$.
  ...
  Note also that $\d_2$ is 0 because the boundary of any 1-dim chain in $K$ is 0.

  Finally, we compute
  \begin{align}
    &\quad\, \homl[1](K) \\
    &\isom \Z^3 / \im(\d_1) \\
    &\isom \freeabel{a,b,c}/\freeabel{-a-b+c, a-b-c} \\
    &\isom \freeabel{a,b,c \st -a-b+c = 0, a-b-c = 0} \\
    &\isom \freeabel{a,b \st a-b-c = 0} \\
    &\isom \freeabel{a,b\rangle /\langle -2b} \\
    &\isom \Z \dsum \Z_2
  \end{align}.
...
\begin{defn}
  A cycle in $X$ is \de{hollow} if it is not the boundary of some other chain in $X$.
\end{defn}
\begin{rmk}
...
When using normal subgroups that correspond to covering spaces, you get
...
won't be symmetric.  It will look like example Hatcher Section 1.3, (1), p.58 for $S^1 \vee
S^1$.
...
\end{defn}
\begin{rmk}
  Unlike a $\Delta$-complex, no injectivity is required on the interior!  (And a homeomorphism is not required on the interior.  See Hatcher p.104 if you want to remember the restrictions of the characteristic maps for simplicial complexes.)
  \begin{itemize}
...
The definition of \de{singular homology} $\homl(X) = \frac{}{}$ is the same as before, but now notice that these $H$'s can be really big!

If $X$, $Y$ are spaces and $f \from X \to Y$, then I have an induced map $$C_n(X) \to C_n(Y)$$ where $$ \sigma \mapsto \sigma.f$$ obviously commutes with $\d$.

\begin{thm}
  $f$ induces a map $$f_\ast \from \homl(X) \to \homl(Y)$$ and $$(f.g)_\ast = f_\ast.g_\ast$$
\end{thm}
...
\end{lem}
\begin{proof}
  By definition of a homotopy equivalence, $f.\phi = \id_{X}$.  It immediately follows that $f_*.\phi_* = (f.\phi)_* = (\id_{X})_* =\id_{\homl(X)}$.  The other result is symmetric to this one.
\end{proof}
\begin{lem}
  If $A \into X$ is a deformation retract, then the inclusion $\homl(A) \into \homl(X)$ is an isomorphism!
\end{lem}
\begin{proof}
  The goal is to show that there is a homotopy equivalence between $X$ and $A$.  We know $\iota \from A \to X$ and $f \from X \to A$.  We also know that $\iota.f = \id_A$.  So $\iota$ and $f$ are homotopy equivalences, and from the previous thoerem, $\iota_*.f_* = \id_{\homl(A)}$, and vice-versa.  But if two spaces are homotopically equivalent, then they have the same homology too. (need more justification here.  also, why do we need a deformation retract instead of just a retraction?)
\end{proof}
\begin{thm}
  The homology of a contractible space $X$ satisfies $\rhoml(X) = 0$ for all $n > 0$.
\end{thm}
\begin{proof}
  By definition, a contractible space $X$ has a weak deformation retract from $X$ to a point $\{p\}$.  By the previous lemma, $\rhoml(X) \isom \rhoml(\{p\}) = 0$.
\end{proof}

\subsection{Reduced homology $\rhoml(X)$}

\begin{defn}
  The \de{reduced chain complex} is obtained from the unreduced chain complex by adding a $C_{-1}(X) :\isom \Z$ to the end:
  $$\to\to\to C_2(X) \to C_1(X) \to C_0(X) \to \Z.$$
\end{defn}
\begin{defn}
  The \de{reduced homology}, denoted $\rhoml(X)$, is merely the homology of the reduced chain complex.
\end{defn}
\begin{prop}
  The reduced homology as a function of the unreduced homology is
  $$\rhoml[i](X) \isom
  \begin{cases}
    \homl[i](X) &\text{if $i > 0$} \\
    \homl[0](X)/\Z &\text{if $i=0$}
  \end{cases}.$$
\end{prop}

Also, $H_0 \isom \rhoml[0](X) \dsum \Z$.

Assume I have $A \subset X$.  How are $\homl(A) \to \homl(X)$ related?
...
% \overset{~}{H_0}(X/A) \lto(by p_*) \overset{~}{H_0}(X) \lto(by i_*) \overset{~}{H_0}(A) \lto(by \delta) \overset{~}{H_1}(X/A) \lto(and repeat!) \overset{~}{H_1}(X) \lto \overset{~}{H_1}(A) \lto \overset{~}{H_2}(X/A) \lto \overset{~}{H_2}(X) and so forth!
% (see below)

\begin{thm}
  $\delta \from H_{i+1}(X/A) \to \homl(A)$ is a homomorphism.
...
\begin{thm}
  ...
  \begin{enumerate}
    \item $\rhoml[i](X) = 0$
    \item $\rhoml[n](X/A) \isom \rhoml[n-1](A)$
    \item $\rhoml[0](X/A) = 0$
  \end{enumerate}
\end{thm}
\begin{proof}
  \mbox{}
  \begin{enumerate}
    \item Any cycle that contracts is homotopic to the trivial cycle.
    \item Use the LES sequence in the previous theorem, and plug in $\rhoml[i](X) = 0$.  The result, for each $i \neq 0$, is the Very Short Exact Sequence (VSES) $0 \to \rhoml[n](X/A) \to \rhoml[n-1](A) \to 0$.  Therefore the homomorphism in the middle must be an isomorphism.
    \item We know $\rhoml[0](X) = 0$.  But $X/A$ only collapses $X$ further.  It certainly cannot increase the number of path components!
  \end{enumerate}
\end{proof}
\begin{lem}
  For any forcible disjoint union, $$\homl(X \dunion Y) \isom \homl(X) \dsum \homl(Y).$$
\end{lem}
\begin{example}
  In application,
  \begin{align}
    &\quad \rhoml[m](S^n) \\
    &\isom \rhoml[m](D^n/S^{n-1}) \\
...
$\homl(A,B) \to \homl(X,B) \to \homl(X,A) \to$ ...

Note that $\homl(X) = \homl(X, \null)$.
...
\begin{thm}[Excision]
  ...
  \[Z \subset \interior{A} \subset X,\]
  then the inclusion
  \[(X \setminus Z,A \setminus Z) \hookrightarrow (X,A) \]
  induces an isomorphism of homology groups.
\end{thm}

If we have \(X=\interior{A} \cup \interior{B}\), then the inclusion \((B, A \cap B) \into (X,A)\) induces an isomorphism on homology groups as well.
...
\begin{defn}[Need to dbl check this in Hatcher]
  The \de{barycenter operator} $b_\lambda$ for some $n$-simplex $\lambda = [0,,,n]$ is $$b_\lambda(\lambda) = b_\lambda([0,,,n]) := [b, 0,,,n]$$ where $b$ is the barycenter of $\lambda$.
\end{defn}
...
\begin{example}
  ...
  $$S(I) = b(S \d I) = b(S ([v_1] - [v_0])) = b([v_1] - [v_0]) = [b,v_1] - [b,v_0]$$
\end{example}
...
\begin{defn}
  Given a chain complex $\cdots \to C_{n+1} \overset{\d}{\to} C_n \overset{\d}{\to} C_{n-1} \to \cdots$, a \de{chain map} is a map $f \from \chains[\bullet] \to D_\bullet$ which induces a map to another chain complex $\cdots \to D_{n+1} \overset{\d}{\to} D_n \overset{\d}{\to} D_{n-1} \to \cdots$ s.t. $$f \d = \d f.$$
\end{defn}
\begin{thm}
  $S$ is a chain map.
\end{thm}
\begin{proof}
  ...
  \begin{itemize}
    ...
    We expand $S$ by its definition and then apply $\d b = 1 - b \d$:
...
\end{proof}
\begin{defn}
  The \de{table} function $T \from LC_n(V) \to LC_{n+1}(V)$ is defined recursively by
  $$T(\null) = 0$$
...
again, $S \d = \d S$ and $\d T + T \d = 1 - S$

$S \from C_n(X) \to C_n(X)$ inducing isomorphism on homology.
...
\renewcommand{\homlindexdefault}{i}
...
\begin{proof}
  $$\begin{tikzcd}
    &(X, A) \arrow[d] \arrow[r] &(X/A, A/A) \arrow[d] \\
    &(X, V) \arrow[r] &(X/A, V/A) \\
...
$$\dhoml(X) \overset{\iota}{\to} \homl(X)$$
where we think of a simplicial chain as a singular chain.

\begin{thm}
  If $A \subset X$ is a closed (if you include a simplex, also include its faces) subspace and a sub-$\Delta$-complex, then
  $$\begin{tikzcd}
    &\, \arrow[r] &\dhoml(A) \arrow[d, "\iota"] \arrow[r] &\dhoml(X) \arrow[d, "\iota"] \arrow[r] &\dhoml(X, A) \arrow[d, "\iota"] \arrow[r] &\dhoml[i-1](A) \arrow[d, "\iota"] \arrow[r] &\, \\
...
\end{thm}
\begin{proof}
  Assume $\gamma(C) = 0$.  Since $\delta$ is an isomorphism, then $c = \d b$.
...
\end{thm}
\begin{proof}
  It is enough to prove the statement for finite dimensional \D complexes since $\dhoml(X_{i+1}) \isom \dhoml(X)$.

  We now induct on the dimension $k$ of a \D complex:
  \begin{itemize}
    \item Base Case ($k=0$):

    $X = \{\text{points}\}$

    $$\dhoml[0](X) = \Z^{|\{\text{points}\}|} = \homl[0](X)$$
    $$\dhoml(X) = 0 = \homl(X)$$

    \item Inductive Step ($k>0$):

    Assume $X$ is $k$-dimensional.  $X = X_k \supset X_{k-1}$.  Our goal is to use the 5 Lemma.  I will draw the diagram now and justify the isomorphisms after:

    $$\begin{tikzcd}
      &\dhoml[i+1](X, X_{k-1}) \arrow[d, "\isom"] \arrow[r] &\dhoml(X_{k-1}) \arrow[d, "\isom"] \arrow[r] &\dhoml(X) \arrow[d] \arrow[r] &\dhoml(X, X_{k-1}) \arrow[d, "\isom"] \arrow[r] &\dhoml[i-1](X_{k-1}) \arrow[d, "\isom"] \\
      &\homl[i+1](X, X_{k-1}) \arrow[r] &\homl(X_{k-1}) \arrow[r] &\homl(X) \arrow[r] &\homl(X, X_{k-1}) \arrow[r] &\homl[i-1](X_{k-1})
    \end{tikzcd}$$

    Isomorphisms 2 and 4 come from the inductive hypothesis.  As for isomorphisms 1 and 3...

    $$\dhoml(X, X_{k-1}) \isom \begin{cases}
      \Z^n &\text{if $i=k$} \\
      0 &\text{otherwise}
    \end{cases}$$ where $n$ is the number of simplices of dimension $k$.
    ...
    By the 5-lemma, the middle vertical arrow is also an isomorphism!
  \end{itemize}
\end{proof}
...
\begin{thm}[Hatcher, p.140]
  If $X$ is connected, then the boundary map $d_1 \from \Delta_1 \to \Delta_0$ is 0.
\end{thm}
\begin{proof}
  If $X$ is connected, then
  $$\Z = \homl[0](X) = \ker(d_0)/\im(d_1) = \Delta_0/\im(d_1) =? \Z/\im(d_1)$$

  which implies that $\im(d_1) = 0$.
\end{proof}

\begin{thm}
  All closed 2-manifolds are $S^2$ or $T^2 \#\#\# T^2$ if orientable, or $\R P^2 \#\#\# \R P^2$ if nonorientable.
\end{thm}

For $M_n = n$ $T^2$'s, then we have the CW-complex representation with the regular $2n$-polygon lined with edges $aba^{-1}b^{-1}$ $n$ times, with different pairs $(a,b)$ each time.

The CW-complex has 1 0-cell, $2n$ 1-cells, and 1 2-cell.  Therefore, the chain-complex is
$$\begin{tikzcd}
  &0 \arrow[r, ""] &\Z \arrow[r, ""] &\Z^{2n} \arrow[r, "0"] &\Z \arrow[r, ""] &0
\end{tikzcd}$$
...
We have 1 0-cell, $2n$ 1-cells, and 1 2-cell.  Therefore, the chain-complex is
$$\begin{tikzcd}
  &0 \arrow[r, ""] &\Z \arrow[r, ""] &\Z^{2n} \arrow[r, "0"] &\Z \arrow[r, ""] &0
\end{tikzcd}$$
...
\begin{defn}
  Let $X$ be a finite CW-complex.  Then the \de{Euler characteristic} is
  $$\chi(X) := \sum_{i=0}^\oo (-1)^i \#\{\text{i-cells}\}. $$
\end{defn}

\begin{thm}
  $\chi(X)$ is a topological invariant (invariant under homeomorphism).  In particular,
  $$ \chi(X) = \sum_{i=0}^\oo (-1)^i \rank(\homl[i](X)). $$

  Note that $\rank(\homl[i](X)) = \dim_{\Q}(\Q \dsum \homl[i](X))$.
\end{thm}
\begin{proof}
  ...
  implies $\rank(Z_i) = \rank(B_i) + \rank(\homl[i])$.  Also, the SES
  ...
  $$ \rank(C_i) = \rank(B_i) + \rank(\homl[i]) + \rank(B_{i-1}).$$
  ...
  \begin{align}
    ...
    \chi(X) &= \sum_{i=0}^N (-1)^i (\rank(B_i) + \rank(\homl[i]) + \rank(B_{i-1})) \\
    \chi(X) &= \rank(B_{-1}) + \left(\sum_{i=0}^\oo (-1)^i \rank(\homl[i])\right) + \rank(B_N)
  \end{align}
...
\renewcommand{\homlindexdefault}{k}

\begin{defn}
  Let $G$ be an abelian group.  Then the \de{n-chains with $G$-coefficients}, denoted $C_n(X; G)$, is the set of $G$-linear combinations of singular $n$-simplices in $X$.
\end{defn}
\begin{prop}
...
Also, the analogue of the CW chain complex

$$\to \to \cwhoml[i](X^i, X^{i-1}) \to \cwhoml[i-1](X^{i-1}, X^{i-2}) \to \to$$
$$\to \to \Z^{\#\{\text{$i$-cells}\}} \to \Z^{\#\{\text{$i-1$-cells}\}} \to \to$$

is the CW chain complex with G coefficients

$$\to \to \cwhoml[i](X^i, X^{i-1}; G) \to \cwhoml[i-1](X^{i-1}, X^{i-2}; G) \to \to$$
$$\to \to G^{\#\{\text{$i$-cells}\}} \to G^{\#\{\text{$i-1$-cells}\}} \to \to$$
...
\begin{example}
  ...
  \begin{defn}
    ...
    this definition in the next lecture. (Note: Tor stuff is located in Hatcher p.263--267)
  \end{defn}
...
\begin{defn}
  For $A$ an abelian group presentation of $A$, a \de{free resolution} of $A$ is an exact sequence of free abelian groups ending in $A \to 0$:
...
\mydate{dm}{21}{4}{2017}
\renewcommand{\homlindexdefault}{n}
\subsection{Mayer-Vietoris sequence}

This is a long exact sequence similar to the one given by the snake lemma.

Let $A$ and $B$ be subspaces of $X$ such that $X = \interior{A} \union \interior{B}$.  We know
...
$$\begin{tikzcd}
  &\, & \homl(A) \arrow[rd, "j_*"] \\
  &\homl(A \cap B) \arrow[ru, "i_*"] \arrow[rd, "-i'_*"] & \oplus
  &\homl(X) \arrow[r,"\partial"]& \ \\
  & &\homl(B) \arrow[ru, "j'_*"] \\
  &\, & \homl[n-1](A) \arrow[rd, "j_*"] \\
  &\homl[n-1](A \cap B) \arrow[ru, "i_*"] \arrow[rd, "-i'_*"] & \oplus
  &\homl[n-1](X) \\
  & &\homl[n-1](B) \arrow[ru, "j'_*"]
\end{tikzcd}$$
Thus, if \(c \in \homl(X)\), then \(c = c' + c''\) and so \(\d c
= 0 \implies \d c' = -\d c ''\). This tells us that
$$ \homl(A \cap B) \to \homl(A) \dsum \homl(B) \to \homl(X) $$

is exact.  Thus, since $\d$ sends \(\homl(X)\) to \(\homl[n-1](X)\), we get a long exact sequence like in the snake lemma!
\begin{example}
  Take \(X = S^n\), \(A\) to be a neighborhood of the top hemisphere,
  and \(B\) to be a neighborhood of the bottom sphere. Then, \(A
  \intersect B\) is an equatorial band that retracts onto \(S^{n-1}\). So,
...
\begin{thm}
  If we have a LES with finitely many nonzero terms...

  $0 \lto M1 \lto M2 \lto M3 \lto \lto \lto 0$

  ...then the alternating sum (meaning multiply every other dim by negative 1 before adding them) of dim is 0.


  $M_1$'s are VS over a field
  $d$'s a linear maps

  $\dim(M_1) = \rk(d_1)$.  (now repeatedy apply rank nullity theorem...)
  $\dim(M_2) = \rk(d_1) + \rk(d_2)$
  $\dim(M_3) = \rk(d_1) + \rk(d_2) + \rk(d_3)$
  ...
\end{thm}
...
\mydate{do}{26}{4}{2017}
\renewcommand{\homlindexdefault}{m}


\begin{thm}[Hatcher Prop 2B.1(a), p.169]
...
\begin{proof}
  ...
  \end{itemize}
\end{proof}
\begin{cor}[Hatcher Prop 2B.1(b), p.169]
...
$k = n-1$.  Our corollary shows $S^n - h(S^{n-1})$ has two components.  But are they disks?  No!

\begin{example}[Hatcher Example 2B.2, p.170]
  $k = 2, n = 3$.
...
% see http://ctan.math.utah.edu/ctan/tex-archive/macros/latex/contrib/amsrefs/amsrdoc.pdf
  ...
  \bib{heq}{webpage}{
    title={Algebraic Topology, Homotopic Functions and Homotopy Classes},
    url={http://www.mathreference.com/at,topy.html},
