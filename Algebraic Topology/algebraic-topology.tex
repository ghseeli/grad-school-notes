...
\end{prop}
  ...
  \begin{thm}
    For every dimension $n$ and degree $m$, there exists a map of degree $m$ from $S^n$ to $S^n$.
  \end{thm}
  \begin{example}
    For any $m$, a map $S^2 \to S^2$ may be obtained by

    <pretty picture>

    cutting out $m$ open disks on $S^2$, and sending each homeomorphically to all of $S^2$ minus a single point.  All other points get sent to that single point on $S^2$, which is the boundary of the image of the open disks.
  \end{example}
