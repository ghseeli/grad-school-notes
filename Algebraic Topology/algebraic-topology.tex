...
$\fund\left( \frac{X_1 \union D^2}{\sim} \right) \isom \frac{\fund(B) * \fund(A)}{\fund(A \cap B)} \isom \frac{\fund(X_1)}{[\phi] = 1}$, where * denotes the free group product. 

In general, I have many gluing maps $\phi_\alpha\from S^1 \to X_1$ and I glue $D^2_\alpha$ along $\phi_\alpha(S^1)$.  So $\fund(X_2) \isom \fund(X_1)/\langle[\phi_\alpha]\rangle$.  ((this one makes my head spin.  i dont get it))
...
\fund(\widetilde{M}) \isom \frac{\fund(M \setminus U) * \fund(D^2
      ...
      S^{n-2}))} \siom \frac{\fund((M \setminus U) \# (D^2 \times
