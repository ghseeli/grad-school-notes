...
\begin{defn}
  ...
  That is, if $a \colon x \to x'$, then there exists $a^{-1} \colon x' \to x$
...
\begin{prop}
  ...
  to the functor $$f_* : \Pi_1(X, x) \to \Pi_1(Y, f(x)).$$
...
\begin{defn}
  ...
  and map \(p \colon \tilde{X} \to X\). Where there exists a cover
