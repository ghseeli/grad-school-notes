...
Consider the polygon (\designateref{*})
...
That's because for any dimensional disk, it's boundary is connected, so that boundary must lie entirely in \textbf{one} of the connected components.

(referring to example \ref{*})

set of connected components = $\pi_0(x_0) = \pi_0(x_1)$

If x_0 = \{[*]\},

$\pi_1(x_1) \isom F_{\text{set of 1-cells}}

$\pi\left( \frac{x_1 \union D^2}{\sim} \right) \isom \frac{\fund(B) * \fund(A)}{A \cap B} \isom \frac{\fund(x_1)}{[\phi] = 1}$

In general, I have many gluing maps $\phi_\alpha\from S^1 \to X_1$ and I glue $D^2\alpha$ along this $\fund(x_2) \isom \fund(x_1)/\langle[\phi_\alpha]\rangle$.

\begin{thm}[Not to be proven]
  Any group is the fundamental group of some CW complex.  Construct as follows:
...
\end{thm}
\begin{thm}
  Every finitely presented group is the fundamental group of a finie CW complex.
\end{thm}
\begin{thm}
...
\begin{proof}
  ...
  $F_n \isom \fund(#^n(S^{n-1 \x S^1})) = \fund(M # N) \isom \fund(M) * \fund(N)$ (where \fund(M) * \fund(N) is the free product of groups)

  and $\delta(S^0 \x D^n) = S^0 \x S^{n-1} - \delta(D^1 \x S^{n-1})$
...
\begin{proof}
  ...
  B = complement of a small disk \homotopic x^{n-1}

  S^{n-1} \to x^{n-1}.

  A \intersect B \homotopic S^{n-1}
\end{proof}
