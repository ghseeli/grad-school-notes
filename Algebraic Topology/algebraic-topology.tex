...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{align}
      &\quad\, \d T (\lambda) \\
      &= \d b(\lambda - T \d (\lambda)) \\
      &= \lambda - T \d (\lambda) - b \d(\lambda - T \d (\lambda)) \\
      &= \lambda - T \d (\lambda) - b (S \d (\lambda)) \\
      &= \lambda - T \d (\lambda) - S (\lambda)
    \end{align}
  \end{itemize}
  Rearranging, we end with the desired
  $$\d T (\lambda) + T \d (\lambda) = \lambda - S (\lambda),$$
...
\begin{defn}
  Let $G$ be an abelian group.  Then the \de{n-chains with $G$-coefficients}, denoted $C_n(X; G)$, is the set of $G$-linear combinations of singulart simplices in $X$.
\end{defn}
\begin{prop}
