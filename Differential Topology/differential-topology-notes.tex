...
\begin{example}
	...
	$p$ extends to $\bar{p} \from \C P^1 \to \C P^1$, where $\C P^1$ isom $\C^* \isom mS^2$.
...
\end{defi}
\begin{defi}
	Given the function $f\from A \to B$, the \de{graph} of $f$ is $$\Gr(f) := \{ (x, f(x)) \mid x \in A \}$$
\end{defi}
\begin{rmk}
...
\end{rmk}
\begin{defi}
	$f$ is \de{Lefschetz} if it is a smooth map of manifolds $f\from M \to M$ and (TFAE)
	\begin{itemize}
