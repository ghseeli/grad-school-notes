...
\end{defi}
\begin{prop}[Equivalent definition]
...
% \mydate{d19}{5}{12}{2016}

% \begin{defi}
% 	This is not a rigorous definition
% 	r-forms
% 	Let W denote capital omega!
% 	\Omega^r(M) denotes the set of r-$forms \in M$

% 	general r-forms
% 	$w \in \Omega^r(M)
% 	Then w(x) : (T_xM)^r to \R s.t. w satisfies
% 	  * w(x) is linear in each variable (an r-linear function)
% 	  * w(x) is "alternating"...

% 	For any permuatation sigma,
% 	  w(x)(vsigma(1),,,vsigma(r)) = sign(sigma)w(x)(v_1,,,vr)

% \end{defi}

% \begin{example}
% 	0-form
% 	f: M to \R <-- one such 0-form
% 	\Omega^0(M) = C^\infty(M) <-- the set of 0-forms
% \end{example}
% \begin{example}
% 	1-form
% 	a smooth section of (TM)* = cotangent bundle of M
% 	$w \in \Omega^1(M), $x \in M$, w(x): T_xM to \R
% \end{example}
% \begin{example}
% 	1-form
% 	w(x) = f1(x)dx1 +++ fn(x)dxn
% \end{example}

% \begin{example}
% 	If U is an open set in \R^n, then \Omega^1(U) is the free C^\infty(M) module on generators with basis {dx1,,,dxn}
% \end{example}

% \begin{example}
% 	d is the differentiation map.  then
% 	$$\begin{tikzcd}
% 	 	& \Omega^0(M) \arrow[r, "d (grad)"] & \Omega^1(M) \arrow[r, "d (curl)"] & \Omega^2(M) \arrow[r, "d (div)"] & \Omega^3(M) \arrow[r, "d"]
% 	\end{tikzcd}$$
...
% 	$$\begin{tikzcd}
% 	 	& f \arrow[r, "d"] & sum_{i=1}^{n} \frac{df}{dx}i dxi \arrow[r, "d"] & sum_{i=1}^{n} sum_{j=1}^{n} d2f/(dxi dxj) dxi^dxj = 0
% 	\end{tikzcd}$$

% 	but since dxi^dxj = -dxj^dxi, the last one is 0.

% 	lookup div, grad, curl, and understand that d is the generalization of them.
% \end{example}
% \begin{prop}
% 	for any r,
% 	d.d = 0
...
% \begin{example}
% 	i THINK that an r-form is like a function f:M to \R with the total derivative d applied to it r times.
...
% 	v_1,,,vn are each n-dimensional vectors
% 	w(x)[v_1,,,vn] = det(v_1...vn)

% \begin{example}
% 	2-forms
% 	w(x) = f1(x,y,z)dx^dy + f2(x,y,z)dx^dz + f3(x,y,z)dy^dz
% 	in fact, i think f1(x,y,z)dx^dy by itself is a 2-form
% \end{example}
% \begin{prop}
% 	(Matt)
% 	diff r-forms are additively closed
% \end{prop}
% \begin{example}
% 	integration
% 	(this works for any r, not just 3)
% 	w = g dy1 ^ dy2 ^ dy3
% 	INT w = INT INT INT g(y1,y2,y3) dy1 dy2 dy3 (no longer wedge on this side)

% 	define
% 	f*(w)(x) := g(f(x)) det(dfi/dxj(x)) dx1 ^^^ dxn

% 	where the det thing is the Jacobian!
% \end{example}

% \begin{thrm}
% 	f is an orientation preserving diffeomorphism V to U.  where both are open in \R^n
% 	and $w \in \Omega^n(U)
% 	f*(w) in \Omega^n(V)
% 	then
...
% 	INT INT INT_U g(y) dy1...dyn = INT INT INT_V f(g(x)) det(dfi/dxj(x)) dx1... dxn
% 	change of variable formula
% 	.
% \end{thrm}
% \begin{defi}
% 	The \de{closed r-forms} on M is $$ ker(d: \Omega^r(M) to \Omega^{r+1}(M) ) = { w | dw = 0 }  $$
% \end{defi}
% \begin{defi}
% 	The \de{exact r-forms} on M is $$ Im(d: \Omega^{r-1}(M) to \Omega^{r}(M) ) = { w | there exists n s.t. dn = w }  $$
% \end{defi}
% \begin{prop}
% 	Every exact form is closed
% \end{prop}
% \begin{defi}
% 	H^r_{Derham}(M) = (closed r-forms)/(exact r-forms)
% 	This is a real vector space modded out by a subspace
% \end{defi}
% \begin{thrm}
% 	stokes thm
% 	INT_M dw = INT_{dM} w
