...
\begin{defi}
	$X$ is \de{connected} if whenever $X = A \cup B$ ($A$ and $B$ are both open and nonempty), then $A \union B$ is nonempty.
\end{defi}
...
\begin{defi}
	...
	$m \leq n$.
	A smooth function $f \from M \to N$  is an \de{immersion} if given any $a, b$ such that $\phi_a\from U_a \to \R^m$, $\psi_b\from V_b \to \R^n$, and any $x \in \phi_a(U_a \union V_b.f^{-1})$, then $x.(\phi_a^{-1}.f.\psi_b)'$ has rank $m$.
\end{defi}
...
\begin{example}
  ...
  ``positive'' if \(x, v_1, \ldots, v_{n-1}\) is standard for \(\R^n\).
\end{example}
\begin{example}
...
\begin{defi}
  ...
  Let \(x \in K\). Then, we have that \[
    T_x K \into T_x M \overset{df_x}{\onto} T_y N
  \]
