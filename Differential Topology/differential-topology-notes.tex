...
\end{thrm}
\begin{proof}
	(see \url{https://proofwiki.org/wiki/Retraction_Theorem} for a complete proof)
	Suppose there was.  Then let $y$ be a regular value for $r$.  Regular for $\d r = \text{id}$.
	So $r^{-1}(y)$ is a compact 1-dim manifold with boundary.  So $r^{-1}(y)$ is a single point.
\end{proof}
...
\end{defi}
\begin{prop}
	The above is well-defined!  (you get the same cardinality for all such $g$!)
\end{prop}
% \begin{proof}
