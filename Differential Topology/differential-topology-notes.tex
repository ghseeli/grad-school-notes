...
\begin{defi}
	...
	(recall $\text{Fix}(f) := \text{the fixed pts of f}$)
	recall
\end{defi}
\begin{thrm}
	$f$ is Lefschetz iff (put midterm thrms here for that problem)
	iff
	..is an eigenvalue for $df_x - I : T_xM \to T_xM$


	$f$ is Lefschetz $L(f) = \sum_{x \in \text{Fix}(f)} o(x,x)$ (maybe that's o((x,f(x))), a point in Gr(f) !!!)
\end{thrm}


\begin{lem}
	$o(x,x) =$ orientation number = +1 if $df_x - I$ preserves orientation, -1 otherwise.
\end{lem}
\begin{thrm}
	If $f$ is Lefschetz, then $L(f) = \sum_{x \in \text{Fix}(x)} \text{sign}(df_x - I)$.

	$$dfx - I : T_xM \overset{~}{\to} T_xM$$
\end{thrm}
\begin{example}
...
\begin{cor}
	...
	If $L(f) \neq 0$, then any $g \homotopic f$ must have at least one fixed point.
\end{cor}


\textbf{Note.}
For any (smooth) vector field, if we let it flow just a little bit, we get a function homotopic to $id$. Since $id$ has fixed points, then the vector field function also has fixed points. This proves the smooth hairy ball theorem!

If $f$ and $g$ are homotopic, and if $f$ has 2 fixed points, then $g$ will have 2 more positive fixed points than negative fixed points.
