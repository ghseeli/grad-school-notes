...
\section{mod 2 intersection numbers (not sure if this is really meant to be a new section or not)}
\begin{defi}
	Given compact manifold $M^m$, and manifolds $M^m, N^n, L^l$ all with empty boundary,

	$$M^m \overset{f}{\to} N^n \supseteq L^l$$

	and $m + l = n$, then the \de{mod 2 intersection number}, denoted $I_2(f, L) \in \Z_2$, is the cardinality of $g^{-1}(L)$ mod 2 where $g \homotopic f$ and $g \transverse L$.
\end{defi}
\begin{prop}
	The above is well-defined!  (you get the same cardinality for all such $g$!)  Also note that at least one such $g$ always exists thanks to the Homotopy Transversality Theorem.
\end{prop}
\begin{proof}
...
\begin{defi}
	$M_0^n$ and $M_1^n$ are \de{cobordant} if they are closed (compact and something) and there exists $W^{n+1}$ s.t. $\d W = M_0 \amalg M_1$.
\end{defi}
\begin{defi}
	$f_0$ and $f_1$ are \de{cobordant} maps if there exists $W$ as before
	$F: W \to N$
	and
	$F|_{M_0} = f_0$.
	Same for $f_1$.
\end{defi}
\begin{prop}(Cobordism)
	Given a cobordism above and any $L$ s.t. $L \subseteq N$ and $l + m = n$, then $I_2(f_0, L) = I_2(f_1, L)$.
\end{prop}
\begin{cor}
	If $M$ is compact without boundary, $M = \d W$, $f : M \to N$ extends to $F: W \to N$, then for all $L \subseteq N$, $I_2(f, L) = 0$.
\end{cor}
\begin{cor}
	If $f,g: M \to N \supseteq L$ and $f \homotopic g$, then their mod 2 intersection numbers are equal.
\end{cor}
\begin{defi}
	If $f : M^m \to N^n$ and $N$ is connected and $M$, $N$ are compact without boundary and $m=n$, then the \de{mod 2 degree} of $f$, denoted $\deg_2(f)$, is $I_2(f, \{y\})$ for any $y \in N$.
\end{defi}
...
\end{proof}
\begin{prop}
	$dwW = M0 \amalg M1$, $F: W \to N$, $m_0 + m_1 = n$, $f_0 = F|_{M_0}$ same for 1.  Then $\deg_2(f_0) = \deg_2(f_1)$.
\end{prop}
\begin{proof}
	Choose a regular value for $F$, $dF$.
	$F^{-1}(y)$ is a 1-manifold with boundary $f_0^{-1}(y) \amalg f_1^{-1}(y)$.
\end{proof}
\begin{cor}
	$M = dW$
	$f: M \to N$
...
\end{cor}
\begin{cor}
	If $f_0 \homotopic f_1: M \to N$, then $\deg_2(f_0) = \deg_2(f_1)$.
	...
	$\deg_2(u_z) = case 0$ if $z$ is outside $M$.
	Case 1 if $z$ is inside $M$.
\end{cor}
\begin{thrm}
