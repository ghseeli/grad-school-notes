...
\begin{defi}[r-forms]
	$\Omega^r(M)$ denotes the set of $r$-forms with respect to $M$.

	General $r$-forms are defined as follows.
	If $w \in \Omega^r(M)$,
	then $$w(x) : (T_xM)^r \to \R$$ is defined such that $w$ satisfies the following:
	\begin{itemize}
	  \item $w(x)$ is linear in each variable (an $r$-linear function).
	  \item $w(x)$ is "alternating," which means that for any permuatation $\sigma$,
		  $$w(x)(v_{\sigma(1)},,,v_{\sigma(r)}) = \sign(\sigma)w(x)(v_1,,,v_r).$$
	\end{itemize}

	Note that if $r$ is $0$, we define $w$ to be smooth s.t. $$w: M \to \R$$ instead.
\end{defi}
