...
\begin{thm}[a-b-dimensional tangent plane]
	Given a function $f : \R^a \to \R^b$ and its slope matrix $f^\prime(v)$ at a vector $v \in \R^a$, then the equation for the plane tangent to $f$ at $v$ is $T_v(w) = f(v) + f'(v)(w-v)$ or, in point-slope form, $\frac{T_v(w)-f(v)}{w-v} = f'(v)$.
