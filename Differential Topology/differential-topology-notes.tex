...
\theoremstyle{mystyle} % this applies to ALL following new theorems % SEE research seminar, week 10 writeup for a possible way to exclude amsthm and [thm].
...
\end{example}
\begin{defi}
	The \de{$n$-sphere}, denoted $S^n$, is $\{x \in \R^{n+1} |\; ||x||=1 \}$. Note: The *surface* of the $n$-sphere has dimension $n$.  $S^n$ can also be defined as the boundary of the closed disk $D^{n+1}$.
\end{defi}
\begin{example}
...
\begin{example}
	A \de{section} of $p\from TS^2 \to S^2$ is a function $s\from S^2 \to TS^2$ such
...
\begin{defi}
	$f \from \R^n \to \R^m$ is \de{differentiable} at $x \in \R^n$ if there exists a linear function $df_x \from \R^n \to \R^m$  such that $$\lim_{h \to 0} \frac{|| (f(x)-f(x+h)) - (df_x(h)) ||}{||h||} = 0.$$  Note that $h$ is a vector, as well as $x$.
\end{defi}
\begin{defi}
...
\end{thrm}
\begin{defi}
	$f$ is an \de{open} map if the image of every open set under $f$ is open.
...
\end{proof}
\begin{defi}
	For any $k \in \N$, a function $f\from \R^n \to \R^m$ is \\\de{$k$-continuously-differentiable}, denoted $f \in C^k$, if all iterated partial derivatives of $f$ of order $k$ exist and are continuous.
\end{defi}
\begin{thrm}
	For any $k \in \N$, the function $f$ has class $C^k$ iff all of its coordinate functions $f_i$ have class $C^k$.
\end{thrm}
\begin{defi}
	Given a set $V$, a \de{distance} is a function $D \from V \times V \to \R$ that satisfies, for all $x, y, z \in V$:
	\begin{enumerate}
		\item $D(x,x) = 0$
		\item If $x \neq y$, then $D(x,y) > 0$
...
\end{defi}
\begin{defi}
	A function $f\from \R^n \to \R^m$ is \de{smooth}, denoted $C^\infty$, if $f$ is $C^k$ for all $k \in \N$.
...
\end{thrm}
\begin{thrm}
	$X$ is a top space and $~$ is an equivalence relation on $X$.  Then there is a topology on $X/~$ (the set of equivalence classes) as follows.  $S \subseteq X/~$ is open iff (let $\text{backwards}(S) \from X/~ \to  X$) backwards(S) is $open \in X$.  [[ $f\from X/~ \to  Y$ is continuous iff $f \of g\from X \to Y$ is continuous.
\end{thrm}
\begin{defi}
	A \de{topological group} is a Hausdorff space $G$ that is also a group, which satisfies:
	\begin{enumerate}
		\item The function $m \from G \times G \to G$ defined by $m((g,h)) = gh$ is continuous.
...
\begin{defi}[derivative of multivariable function]
	The \de{tangent plane} to function $f \from \R^n \to \R^m$ at point $a$, denoted $df_a$ or $f^{\prime}(a)$, is the unique function that satisfies $$\lim_{h\to 0} \frac{||\left(f(a+h) - f(a)\right) - df_a(h)||}{||h||} = 0.$$
\end{defi}
\begin{defi}
	The \de{directional derivative} of $f$ at $x$ in the $v$ direction is $df_x(v)$ which is $\lim_{h\to 0} \frac{f(x+hv) - f(x)}{h}$.
\end{defi}
\begin{defi}
	Let $U$ and $V$ be open sets in $\R^n$ (topological spaces, soon).  A function $f\from U\to V$ is a \de{homeomorphism} iff it is a bijection and both $f$ and $f^{-1}$ are continuous.
	Synonym: bi-continuous function.
\end{defi}
\begin{defi}
	Let $U$ and $V$ be open sets in $\R^n$.  A function $f\from U\to V$ is a \de{$C^r$-diffeomorphism} iff it is a bijection and both $f$ and $f^{-1}$ are $r$-continuously-differentiable.
\end{defi}
\begin{defi}
	Let $U$ and $V$ be open sets in $\R^n$.  A function $f\from U\to V$ is a \de{diffeomorphism} iff it is a bijection and both $f$ and $f^{-1}$ are differentiable.
...
\begin{defi}
	$X$ is \de{connected} if whenever $X = A \cup B$ ($A$ and $B$ are both open and nonempty), then $A \intersect B$ is nonempty.
\end{defi}
...
\begin{defi}
	...
	A cont function $f \from M \to N$ is \de{smooth} if for all $a$, for all $b$, $\phi_a^{-1}.f.\psi_b$ is smooth.  You might call $f$ a \de{mersion}.
\end{defi}
\begin{defi}
	Given the same givens as above.
	$N$ and $M$ are \de{diffeomorphic} if there exists $f \from M \to N$ and $g \from N \to M$ both smooth such that $f.g = id_M$ and $g.f = id_N$.
\end{defi}
...
\begin{defi}
	...
	$m \leq n$.
	A smooth function $f \from M \to N$  is an \de{immersion} if given any $a, b$ such that $\phi_a\from U_a \to \R^m$, $\psi_b\from V_b \to \R^n$, and any $x \in \phi_a(U_a \intersect V_b.f^{-1})$, then $x.(\phi_a^{-1}.f.\psi_b)'$ has rank $m$.
\end{defi}
...
\begin{defi}
	An \de{embedding} is a smooth map of manifolds $f\from M \to N$ s.t. $f \from M \to f(M)$ is a homeomorphism.

	Note: $f$ need not be onto $N$.

	Note: Since a homeomorphism is the isomorphism of topology, then we say $M$ \de{embeds} into $N$ if there exists an isomorphism $f$ s.t. $f(M) \sub N$.
\end{defi}
...
\begin{defi}
	$f$ is a smooth map of manifolds.
	$x \in M$ is a \de{regular} point if $d_x \from T_xM \to T_yN$ is onto.  Otherwise, $x$ is a \de{critical} point.
\end{defi}
...
\begin{defn}[Matt]
	$M$ and $L$ are \de{locally transverse} at $p$ if there exists a neighborhood $U$ of $p$ in $M$ and a neighborhood $V$ of $p$ in $L$ s.t. $U \transverse V$.  $p$ can also be called a \de{transverse intersection} of $M$ and $L$.
\end{defn}
...
\begin{defi}
	Given a differentiable manifold $M$ and the canonical $p\from TM \to M$, a \de{vector field} is a section of $p$.
\end{defi}
\begin{rmk}
...
\end{example}
\begin{defi}
	A \de{derivation} of a commutative algebra $A$ is derivation of the form $D\from A \to A$.
...
\end{defi}
\begin{thrm}[equivalent definition of parallelizable]
...
\begin{defi}
	$V$ is a finite dim real vector space.  Then an \de{inner product} is a function $<,> \from V \x V \to \R$ that is:
	\begin{enumerate}
...
\begin{thrm}[Tietze Extension Theorem]
	$X$ is $T_4$. A $closed \in X$.  If $f \from A \to  \R$ is continuous, then $f$ extends to all of $X$.
\end{thrm}
...
\end{propo}
\begin{proof}
	$U_1 \supseteq X \minus (U_2 \union \union \union U_r) \eqqcolon  C$, which is closed.
...
\end{propo}
\begin{defi}
	Given $\{U_a\}$ a locally finite open cover of $X$, then a \de{partition of unity} subordinate to $\{U_a\}$ is a collection of continuous functions $\phi_a \from X \to  [0,1]$ where for each $a$, the support of $\phi_a$ is contained in $U_a$.
\end{defi}
...
\begin{proof}
	...
	Let $S_i \coloneqq  S \intersect U_i$
...
% Now let's look at when m  \geq  n.
...
\end{example}
\begin{example}
	There exists continuous $f \from \R \to \R \x \R$ which are onto (space filling curves).
\end{example}
...
% 	Start with 1-1 immersion $f \from M^n \to \R^N$

% 	$\pi_a$

% 	claim A:
% 	"If N > 2n + 1, then there exists nonzero a in RN s.t.
% 		1. f\from Mn \to  RN \pi_a <a>perp is still an immersion.
% 		2. f.\pi_a is still 1-1.
% 	If N > 2N, then can still do (1)."


% 		Claim implies there exists 1-1 immersion f \from M^n \to  \R{2n+1}
...
% 	Let g \from TM \to  RN be g(x,v) = df_x(v)

% 	dfx \from T_xM \to  TRN = RN
...
% 				Let {p_i \from M \to  [0,1]} partition of unity w.r.t. {U}
...
\end{thrm}
\begin{lemm}
	Given $M \x S$ instead of $M$, and $F$ instead of $f$ in above thm.  Suppose $F \transverse L$.  Then for almost all $s \in S$, the function $F_s\from M \to N$ by $F_s(x) = F(x,s)$ is transverse to $L$.
\end{lemm}
\begin{defi}
	A property of a function $f \from X \to Y$ is \de{stable} if for every homotopy $H : X \x [0,1] \to Y$, there exists an $\epsilon > 0$ such that for all $t < \epsilon$, $H_t \from X \to Y$ has the property.
\end{defi}
...
\end{thrm}
\begin{defi}
	Two embeddings $f,g \from M \to N$ are \de{isotopic} if they are homotopic via some $H$, and for all $t \in [0,1]$, $H_t$ is an embedding.
\end{defi}
...
\end{propo}
\begin{defi}
	$f$ and $g$ are \de{cobordant} maps if there exists $A$ and $B$ cobordant under $W$ and $F\from W \to N$ such that
...
\end{thrm}
\begin{defi}
	If $f \from M^m \to N^n$ and $N$ is connected and $M$, $N$ are closed and $m=n$, then the \de{mod 2 degree} of $f$, denoted $\deg_2(f)$, is $I_2(f, \{y\})$ for any $y \in N$.
\end{defi}
...
\end{example}
\begin{defi}
	If $M$ is a smooth $n$-dimensional manifold, then an \de{orientation} of $M$ is a choice of orientation for each $T_xM$ compatible as follows.  $M$ has an atlas of coordinate charts $\{ \phi \from U \to \R^n \mid d\phi_x \from T_xM \to T\R^n = \R^n \text{ is orientation preserving} \}$.
...
\begin{propo}[Equivalent definition]
	A manifold is orientable if and only if there exists an atlas $\{ \phi \from U \to \R^n \mid d\phi_x : T_xM \}$ s.t. for all $i,j$ we have $$\phi_i^{-1}.\phi_j \from \phi_i(U_i \intersect U_j) \to \phi_j(U_i \intersect U_j),$$
...
\begin{defi}
  If \(M,N\) are oriented manifolds, we defined the \de{product
    orientation} on \(M \times N\) by saying, if \((v_1, \ldots,
...
\begin{defi}
  ...
  orient \(\d M\) using the \de{boundary orientation} as follows.
  \begin{itemize}
...
\begin{defi}
  ...
  choice of orientation on \(M\) determines a \de{pre-image orientation} on \(K\) as
  follows. \\
...
\begin{defi}
  ...
  and \(L\) are all oriented, they induce a \de{pre-image orientation}
  on \(K\) as follows. \\
...
\begin{defi}
  ...
  that \(f(x) = y\), we say the \de{pre-image orientation} on \(x\) is +1 if the
...
\begin{defi}
  ...
  = y \in L\), \(x\) gets the \de{pre-image orientation} \(+1\) if, given positive
...
\begin{defi}
	...
	the \de{intersection number} of $f$ and $L$, denoted $I(f,
...
\begin{defi}
	If $\mathcal{H}$, then the \de{degree} of $f$ is $$\deg(f) \coloneqq  I(f, \{y\})$$ for any $y \in N$.
\end{defi}
\begin{propo}
...
%	Whoah, so degree as we know it is related to $d$-to-$1$
%	functions, which is related to intersection numbers!
...
\begin{defi}
	Given the set $M$, the \de{diagonal} of $M$ is $$\Delta M \coloneqq  \{ (x,x) \mid x \in M \}$$
\end{defi}
\begin{defi}
	Given the function $f\from A \to B$, the \de{graph} of $f$
        is $$\Gr(f) \coloneqq  \{ (x, f(x)) \mid x \in A \} \subset A \times B$$
\end{defi}
\begin{rmk}
...
\end{rmk}
\begin{defi}
	$f$ is \de{Lefschetz} if it is a smooth map of manifolds
...
\end{thrm}
\begin{defi}
	If $f \from M \to M$ is a smooth map of manifolds, $M$ is compact, and $\d M = 0$, then the \de{Lefschetz number} of the map $f$ is $$L(f) \coloneqq  I(\Delta M, \Gr(f))$$
\end{defi}
\begin{propo}
...
\begin{defi}
	The \de{zero} vector field is the vector field defined by $$v_0(x) \coloneqq  (x, 0)$$ for all $x$.
\end{defi}
...
\begin{defi}
	The \de{Hessian} of $f\from \R^n \to \R$ is $$\left( \frac{d^2f}{dx_i dx_j} \right)_{i,j}$$
...
\begin{defi}
	A smooth map of manifolds $f\from M \to \R$ is \de{Morse} if all critical points are generate.
\end{defi}
...
\begin{example}[integration]	% Matt, I added \, for readability
	...
	Define
	$$f^*(w)(x) \coloneqq  g(f(x)) \det\left(\left[\frac{df_i}{dx_j}(x)\right]_{i,j}\right) \,dx_1  \wedge  \wedge  \wedge  dx_n$$
