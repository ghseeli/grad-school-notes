...
\begin{defi}
	...
	\begin{itemize}
		...
		\item $L(f) = |\Fix(f)|$ (defined below, but not sure if this is technically true, since $L(f)$'s existence requires $M$ compact and without boundary.)
	\end{itemize}
\end{defi}
...
\begin{thrm}
	...
	Note that $o(x,x) = o((x,x)) = o((x,f(x)))$ is an element of $\Delta M \cap \Gr(f)$.
\end{thrm}
...
\end{lem}
\begin{thrm}
	If $f$ is Lefschetz, then $L(f) = \sum_{x \in \Fix(f)} \text{sign}(df_x - I)$.
...
\begin{example}
	...
	Note that we can figure out local max, local mins with certainty by taking the gradient / Jacobian, then taking the determinant of that, which is the curvature.  Positive curvature is local max/min.  Negative curvature is saddle.  Zero is flat in any direction (not just tangent).
        ...
        case 1:
			...
			% x is a generate critical point of \del^2(x) = H_p(x) is nonsingular (Hessian).
...
%%%%%%%%%% for Fred for Wed Nov 30:

% \mydate{d19}{30}{11}{2016}

% \begin{example}
% 	(ex for nondegen vec fields)
% x is a nondegenerate 0 for v iff d\bar{v}_x is invertible.
% \end{example}
% \begin{defi}
% 	f : \R^n to \R
% 	x is \emph{critical} if \frac{df}{dx}i = 0 for all i from 1 to n
% \end{defi}
% \begin{defi}
% 	A critical point x is \emph{nondegenerate} if H_p(x) = ( d2f/dxidxj )i,j is invertible.
% \end{defi}
% \begin{thrm}
% 	All Hessians are \emph{symmetric}.
% \end{thrm}
% \begin{defi}
% 	A smooth map of manifolds $f: M \to \R$ is \de{Morse} if all critical points are nondegenerate.
% \end{defi}
% \begin{example}
% 	of generate critical point
% 	* a "sink".  all vectors pointing to the critical point
% 	* a "half-sink" "half-source" given by the V.F. < x, -y >
% \end{example}

% \begin{example}
% 	of degenerate point of a V.F...
% 	f: \R to \R
% 	f(x) = x^2
% 	\dell(f)(x) = 3x^2
% 	critical point for f at x = 0
...
% 	If f: M to \R is a smooth map of manifolds and p is a generate critical point, then near p, f(x_1,,,xn) = x_1^2 +++ xn^2 (maybe minus?) with some change of basis.
% \end{lem}

% \begin{defi}
% 	Let g be the genus of the manifold M = # of holes  (We will *not* define genus rigorously in this course)
% 	chi(M) = 2 - 2g
% \end{defi}
% \begin{cor}
% 	If g neq 1, then every vector field on $M_g$ has a zero.
% \end{cor}
% \begin{thrm}
% 	If M has odd dimension, then chi(M) = 0.
% \end{thrm}
% \begin{proof}
% 		chi(M) = I(\delta M, \delta M) = (-1)^{\dim(M)^2} * I(\delta M, \delta M).
% 	If the dimension of M is odd, then chi(M) = -chi(M).
% 	So chi(M) = 0.
% \end{proof}
% \begin{defi}
% 	index
% The index of a point is (TFAE):
% 	* When you visualize the surface, the number of axis directions in which the surface goes downwards.
% 	* When you look at the (dell f, or the laplacian, or maybe the hessian?)
% 	* the number of negative eigenvalues (of the hessian?)
% 	* When you zoom in and approx f as x_1^2 + x_2^2 - x_3^2 + x_4^2, etc, the number of negative summands
% \end{defi}
