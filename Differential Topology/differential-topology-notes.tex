...
\end{thm}
\begin{defn}
	A \emph{topological space} is a set $X$ and a set $O$ of subsets of $X$ that satisfy the following conditions.  Every element of $O$ is ``open'' by definition.  Further:
	\begin{itemize}
...
\end{thm}
\begin{defn}
	A \emph{topological group} $G$ is a group, a topological space, and has that $m$ and $i$ are continuous for the functions $m : G \times G \to G$ defined by $m((g,h)) = gh$ and $i(g)$ the inverse function.
\end{defn}
