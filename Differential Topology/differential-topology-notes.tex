...
\begin{thm}
	$RP^n \isom RP^{n-1} \cup R^n$.
	\textbf{Proof.}  In $\R^{n+1}$, draw a flat "plane" above the origin.  Each line in $RP^n$ that has a nonzero "z" component will intersect this plane.  So each line maps to its intersection with this plane, and this plane is $R^n$.  The remaining lines are all in $RP^{n-1}$.  Wow!
\end{thm}
	...
	If $f$ is smooth, then we can differentiate, getting the linear map
	$d_x(\phi_a^{-1}.f.\psi_b): R^m \to R^n$

\begin{thm}f is smooth
	$f$ is an \emph{immersion} if $d_x$ above is injective.  (this is *local* injectivity of f) (m <= n)
\end{thm}

\begin{defn}
	$f$ is a \emph{submersion} if $d_x$ above is surjective.  (this is *local* surjectivity of f) (m >= n)
\end{defn}

\begin{defn}
	An \emph{embedding} is an immersion that is injective and proper. (MY DEF)
\end{defn}

\begin{defn}
	An \emph{embedding} $f$ is an immersion s.t. $f : M \to f(M)$ is a homeomorphism. (HIS DEF)
\end{defn}
...
\begin{defn}
	y \in N is a \emph{regular} value for $f$ if for every $x$ in $f^{-1}(y)$, we have that d_{\phi(x)}(\phi^{-1}.f.\psi) is ONTO.  If y is not regular, it is a \emph{critical} value instead.  In which case, x is a \emph{critical} point.  Note: values $y$ *not* in the image of $f$ are vacuously regular.
\end{defn}



\begin{thm}
	This comes from the Local Submersion Theorem.  If y \in N is a regular value, then f^{-1}(y) is a smooth submanifold of M of dim m-n.
\end{thm}

\begin{example}
	Mn(R) vectors of dim n^2.  Sn(R) the symmetric matrices.  Note that the matrix A^T A is always symmetric.  O(n) group ortho matrices.

	Let f from Mn(R) to Sn(R) given by f(A) = A^T A.

	THEN I in Sn(R) is a regular value.  Here's why...

	Corr. O(n) = finverse(I) is a smooth manifold of dimension n^2 - n(n+1)/2 = n choose 2.
...
Given A in Mn, find a formula d_Af from Mn to Sn ...

df_A(B) = lim f(A+tB) - f(A) over t = (.... = B^T A + A^T B
is indeed in Sn! (it is symmetric)

If A^T A = I, can every symmetric matric C be written in the form C = B^T A + A^T B.

sneaky trick
Guess A^T B = 1/2 C.
This is possible if we let B = 1/2 AC.
Then it works!
