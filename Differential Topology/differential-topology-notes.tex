...
\begin{thrm}[a-b-dimensional tangent plane]
...
\end{defi}
\begin{defi}
	A topological space $X$ is \emph{Hausdorff} (or \emph{T1}) if for every $a \neq b$ in $X$, there exists an open neighborhood $A$ of $a$ and an open neighborhood $B$ of $b$ such that $A \cap B = {}$.  Synonyms: seperated.  Notation: $T_2$ is the set of all Hausdorff topological spaces.
...
\begin{defi}
	A space is \emph{2nd-countable} if it has a countable basis.
\end{defi}
\begin{defi}
...
\begin{thrm}
	If $f$ is transverse to $L$, then $ \dim(M) - \dim f^{-1}(L) = \dim(N) - \dim(L)$.
...
\begin{lem}
	...
	Then
	$D_w = D_v of \phi^* : C_\infty(\R^n,q) \to \R$

	$M$ is an $n$-dimensional manifold, $p \in M$, $v \in T_pM$.
	want to define
	$D_v : C_\infty(M,p) \to R$
	given $\phi: M \supseteq U \to \R_n$ let $D_v$ be the composit
	$C_\infty(M, p)$ to by ${\phi^{-1}}^*$ to $C_\infty(\R^n,q)$ to $D_w$ to $\R$.

	independent of coordinate chart, by lemma
\end{lem}
\begin{defi}
	$D: C_\infty(M,p) \to \R$ is a \emph{derivation} if it is linear and $D$ satisfies the product rule.

	The $\Der(M,p)$ is the set of all such derivations.
\end{defi}
\begin{thrm}
